...
def generator(config):
	...
	for r in range(N_regions):
		...
		# Scale indices
		scale_ind = np.arange(0, config.N_levels_per_region * (r+1), r+1)
		# Feature values
		region_feature_vals = all_feature_vals[scale_ind, :]
		...
		dset_name = 'analogy_scale_' + region_name
		filename = config.dir_name + '/' + dset_name + '.hy'
		f = h5py.File(filename, 'w')

		# Directory for drawing images
		check_path('../figures/')
		img_figs_dir = '../figures/' +  'analogy_scale_' + region_name + '/'
