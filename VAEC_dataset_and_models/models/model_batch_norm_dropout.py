...
class Model(object):
	...
	def __init__(self, config, batch_ops, is_train=True):
		...
		# Add dropout
		linear_dropout = 0.5
		keep_prob = 1 - linear_dropout
		A_dropout = slim.dropout(A_batch_norm, keep_prob=keep_prob, is_training=is_train)
		B_dropout = slim.dropout(B_batch_norm, keep_prob=keep_prob, is_training=is_train)
		C_dropout = slim.dropout(C_batch_norm, keep_prob=keep_prob, is_training=is_train)
		D_dropout = slim.dropout(D_batch_norm, keep_prob=keep_prob, is_training=is_train)

		# [A, B, C, D] -> LSTM
		log.info('[A,B,C,D] -> LSTM...')
		D_score = scoring_model(A_dropout, B_dropout, C_dropout, D_dropout)
		...
		for foil in range(N_foils):
			...
			# Dropout
			foil_dropout = slim.dropout(foil_batch_norm, keep_prob=keep_prob, is_training=is_train)
			# Get score
			foil_score = scoring_model(A_dropout, B_dropout, C_dropout, foil_dropout)
